...
class NumberFormat {
    ...
    fun electricityUsageToCarbonUsage(electricityUsage: Float): Float {  //Wh단위일 때 넣으면 된다
        return (electricityUsage * 0.001 * 424).toBigDecimal().setScale(2, RoundingMode.HALF_UP)
            .toFloat()
    }
    ...
    fun totalCarbonUsageString(waterUsage: Float, electricityUsage: Float): String {
        return (waterUsageToCarbonUsage(waterUsage) + electricityUsageToCarbonUsage(electricityUsage)).toBigDecimal()
            .setScale(2, RoundingMode.HALF_UP).toString() + " g"
    }
